...
contract("Land", ([owner1, owner2]) => {
  ...
  describe("Withdraw", () => {
    describe("success", () => {
      it("only owner can withdraw deposit", async () => {
        const balanceBefore = await web3.eth.getBalance(owner1);
        await land.mint(1, { from: owner2, value: COST });
        await land.withdraw({ from: owner1 });
        const balanceAfter = await web3.eth.getBalance(owner1);
      });
    });

    describe("failure", () => {
      it("others cannot call withdraw function", async () => {
        await land
          .withdraw({ from: owner2 })
          .should.be.rejectedWith(EVM_REVERT);
      });
    });
  });
